...
<script type="text/javascript" language="JavaScript">
		...
		for(var ii=0;ii<count;ii++){
			...
			$.ajax({
				...
				success: function(datenum){
					if (datenum.num>0) {
						docload(datenum.typelist,datenum.mid);
	...
	function docload(typeid,mid){

		var typeArray=null;
		typeArray=typeid.split(',');

		var typecount=typeArray.length;
		if (typecount>0){

			var loaddocnumurl="index.php?archive=createmain&action=docnum";
			...
			$.ajax({
				...
				success: function(date){
					if(date>0){
						...
						var limitnum=Math.round(date/10+1);
						creatdoc(0,0,10,typeid,limitnum);
					}
				}
			});
		}
	...
	function creatdoc(limitstart,i,limitnummax,typelist,limitnum){
		var loadurl="index.php?archive=createmain&action=creatdocsave";
		if (i<limitnum){
			$.ajax({
				type: "POST",
				url: loadurl,
				data: "lng=<?php echo $this->_tpl_vars['read']['lng'] ?>&tid="+typelist+"&limit="+limitstart+"&limitnummax="+limitnummax+"&startid=<?php echo $this->_tpl_vars['read']['startid'] ?>&endid=<?php echo $this->_tpl_vars['read']['endid'] ?>&creattype=<?php echo $this->_tpl_vars['read']['creattype'] ?>&dtime=<?php echo $this->_tpl_vars['read']['time'] ?>",
				success: function(date){
					if (date=='true'){
						i++;

						limitstart=i*limitnummax;
						creatdoc(limitstart,i,limitnummax,typelist,limitnum);	
					}
				}
			});	
		}
