...
class uploadFile extends connector {
	...
	function upfilebase($TypeW, $TypeH, $smallTypeid = 0, $img_iswater = 0, $path = 'upfile/', $filename = null, $filesize = 0, $filetmpname = 0, $fileerror = null, $filetype = null, $type = 'img', $dirdate = 'y', $filenametype = 'd', $iszoom = 1) {
		...
		if ($filetmpname != '') {
			...
			$formatfilename = $this->extendname($filename, 2);
			...
			if (preg_match("/^[\w-]+$/u", $formatfilename) && $filenametype == 'f') {
				$filename = strtolower($filename);
			} else {

				$formatfilename = $this->formatname(0);
				$filename = $formatfilename . '.' . $filetype;
			}
			...
			if ($smallTypeid && $type == 'img') {
				...
				$upresultid = $this->upsmallpic($uploadfile, $TypeW, $TypeH, $toImagesFile, $iszoom);
			}
	...
	function waterimg($srcFile, $toImagesFile, $img_iswater = 2) {
		...
		switch ($img_iswater) {
			...
			case 1:
				...
				$img_wmt_size = ceil($this->CON['img_wmt_size']);
	...
	function upsmallpic($srcFile, $dstW, $dstH, $toImagesFile, $iszoom = 1) {
