...
class connector {
	...
	function get_typelist($t_array = array(), $mid = 0, $in_tid = 0, $now_tid = 0, $lng = '', $level = 0, $isclass = 0, $ishtml = 0) {
		$lng = empty($lng) ? $this->CON['is_lancode'] : $lng;
		...
		if (count($t_array) < 1) {
			...
			if (!$t_array) {
				...
				$t_array = $this->dbcache->cachesave('typelist_array_' . $lng . '_' . $mid . '_' . $isclass . '_' . $ishtml, $array);
	...
	function get_typeid($tid = 0, $field_name = 'tid', $retid = 0, $mid = 0, $now_tid = 0, $lng = '', $level = 0, $isclass = 0, $returntype = 'in', $ishtml = 0) {
		$lng = empty($lng) ? $this->CON['is_lancode'] : $lng;

		$getAllTypeid = $this->get_typelist(array(), $mid, $tid, $now_tid, $lng, $level, $isclass, $ishtml);
	...
	function get_subjectlist_array($sid = 0, $mid = 0, $lng = '', $isclass = 1, $limit = 0) {
		...
		if ($isclass > 0) $db_where.=" AND isclass=$isclass";
		if ($mid > 0) $db_where.=" AND mid=$mid";
		if ($limit > 0) $db_limit = " LIMIT 0,$limit";
		$sql = 'SELECT * FROM ' . $db_table . $db_where . ' ORDER BY pid,sid DESC' . $db_limit;
		$chacherray = $this->dbcache->checkcache('subjectlist_array_' . $lng . '_' . $mid . '_' . $isclass . '_' . $limit, false);
		$arrayList = array();
		if (!$chacherray) {
			$rs = $this->db->query($sql);
			while ($rsList = $this->db->fetch_assoc($rs)) {
			...
			$chacherray = $this->dbcache->cachesave('subjectlist_array_' . $lng . '_' . $mid . '_' . $isclass . '_' . $limit, $array);
			$chacherray = $chacherray ? $chacherray : $array;
			unset($array);
		}
