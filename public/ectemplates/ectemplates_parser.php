...
class EctemplatesParser {
	...
	private function _parse_find() {
		if (preg_match('/' . $this->left_delimiter . '\s*find:[0-9a-zA-Z_]+\s*class*=\s*/', $this->template)) {
			preg_match_all('/' . $this->left_delimiter . '\s*find:[0-9a-zA-Z_]+\s*class=[^\s]+\s*out=[^%]+\s*' . $this->right_delimiter . '/', $this->template, $getarray);
			foreach ($getarray[0] as $key => $variable) {
				$bearkarray = array();
	...
	private function _parse_array__find($bearkarray) {
		...
		$this->template = str_replace($bearkarray[0][0], $this->findhash . $bearkarray[1][0] . '|' . $bearkarray[4][0] . '|' . $bearkarray[3][0] . $this->findhash, $this->template);
	}
