...
class important extends connector {
	...
	function onsave() {
		...
		$date = time();
		if ($inputclass == 'add') {
			$db_field = 'pid,lng,typename,content,purview,ispage,isclass,addtime,pagemax,listmax,isaddclass,ismail,putmail,mailcode,ismenu,isseccode,inputtime,iswap,istemplates,templates';
			$db_values = "50,'$lng','$typename','$content',$purview,$ispage,1,$date,$pagemax,$listmax,$isaddclass,$ismail,'$putmail','$mailcode',$ismenu,$isseccode,$inputtime,$iswap,$istemplates,'$templates'";
			...
			$db_where = 'btid=' . $btid;
			$db_set = "typename='$typename',content='$content',purview=$purview,ispage=$ispage,pagemax=$pagemax,listmax=$listmax,isaddclass=$isaddclass,ismail=$ismail,putmail='$putmail',mailcode='$mailcode',ismenu=$ismenu,isseccode=$isseccode,inputtime=$inputtime,iswap=$iswap,istemplates=$istemplates,templates='$templates'";
