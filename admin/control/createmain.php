...
class important extends connector {
	...
	function onstatype() {
		...
		if ($creattype == 1) {
			$typelist = $this->get_typeid($tid, '', 0, $mid, 0, $lng, 0, 1, 'text', 1);
			if (!empty($typelist)) {
	...
	function oncreattypesave() {
		...
		$dirpathDIR = empty($typeread['dirpath']) ? $typeread['dirname'] : $typeread['dirpath'] . '/' . $typeread['dirname'];
		if ($this->CON['is_alonelng']) {
			$dirpath = $dirpathDIR;
		} else {
			$dirpath = $lng_dir . '/' . $dirpathDIR;
		}
	...
	function oncreatdocsave() {
		...
		$limit = $this->fun->accept('limit', 'P');
		$limit = empty($limit) ? 0 : $limit;

		$limitnummax = $this->fun->accept('limitnummax', 'P');
		$limitnummax = empty($limitnummax) ? 10 : $limitnummax;

		$db_table = db_prefix . 'document';
		$sql = 'SELECT did FROM ' . $db_table . $db_where . ' ORDER BY did DESC LIMIT ' . $limit . ',' . $limitnummax;
		$rs = $this->db->query($sql);
		while ($rsList = $this->db->fetch_assoc($rs)) {
