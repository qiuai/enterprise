...
class important extends connector {
	...
	function oninfosave() {
		...
		if ($inputclass == 'add') {
			...
			$db_values = "50,$atid,'$lng','$title','$filename','$url',$adtype,0,$starttime,$endtime,$time,$isclass,$istime,'$content',$islink,$gotoid";
			...
			if (empty($adid)) exit('false');
			$db_where = 'adid=' . $adid;
			$db_set = "atid=$atid,title='$title',filename='$filename',url='$url',adtype=$adtype,starttime=$starttime,endtime=$endtime,istime=$istime,content='$content',islink=$islink,gotoid=$gotoid";
