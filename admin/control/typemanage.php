...
class important extends connector {
	...
	function ontypelist() {
		...
		$sql = 'SELECT c.*,COUNT(a.tid) AS has_c FROM ' . $db_table . ' AS c LEFT JOIN ' . $db_table . ' AS a ON a.upid = c.tid' . $db_where . ' GROUP BY c.tid ORDER BY c.upid,' . $limitkey . ' ' . $limitclass . ',c.tid';
	...
	function ontypeedit() {
		...
		if ($typeread['styleid'] == 4 || $typeread['styleid'] == 2) {

			$gototypelist = $this->get_typeselect($typeread['mid'], $typeread['tid'], $typeread['gotoid'], $typeread['lng'], 0, 1);

			array_shift($gototypelist);
			$this->ectemplates->assign('gototypelist', $gototypelist);
		}
	...
	function onsynchrosave() {
		...
		$typeRoot = empty($typeread['dirpath']) ? $typeread['dirname'] : $typeread['dirpath'] . '/' . $typeread['dirname'];
		...
		while ($rsList = $this->db->fetch_assoc($rs)) {
			...
			foreach ($db_field1_install_array as $key => $value) {
				$valuestr = $this->fun->daddslashes($rsList[$value], 1);
				$db_values1.="'" . $valuestr . "',";
			}

			$db_values1.="'$typeread[lng]','$totid','',0,0,'',0,'','','','',0,'$typeRoot'";
			...
			while ($rsAlbum = $this->db->fetch_assoc($albumrs)) {
				$picname = $this->fun->daddslashes($rsAlbum['picname'], 1);
				$filedes = $this->fun->daddslashes($rsAlbum['filedes'], 1);
				$db_values2.="($insert_id,'$picname','$filedes','$rsAlbum[picfile]',$rsAlbum[addtime]),";
			}
			...
			if (is_array($rsatt) && count($rsatt) > 0 && !empty($db_field3)) {
				...
				foreach ($db_field2_install_array as $key => $value) {
					if (!empty($value)) {
						$valuestr = $this->fun->daddslashes($rsatt[$value], 1);
						$db_values3.="'" . $valuestr . "',";
					}
				}
			...
			$contentstr = $this->fun->daddslashes($rsContent['content'], 1);
			$this->db->query('INSERT INTO ' . $db_table4 . ' (' . $db_field4 . ") VALUES ('$insert_id','$contentstr')");
		}
		exit('true');
	}
	...
	function ontypesave() {
		...
		if ($inputclass == 'add') {
			...
			$db_field = 'pid,mid,topid,upid,exmid,lng,typename,content,keywords,description,typepic,dirname,purview,ismenu,isaccessory,isclass,ispart,styleid,pageclass,
				indextemplates,template,readtemplate,filenamestyle,readnamestyle,typeurl,dirpath,addtime,iswap,waptempalte,wapreadtemplate,pagemax,isline,gotoline,isorderby,ordertype';
			...
			'$indextemplates','$template','$readtemplate','$filenamestyle','$readnamestyle','$typeurl','$dirpath',$date,$iswap,'$waptempalte','$wapreadtemplate',$pagemax,$isline,$gotoline,$isorderby,$ordertype";
			...
			readnamestyle='$readnamestyle',typeurl='$typeurl',indextemplates='$indextemplates',iswap=$iswap,waptempalte='$waptempalte',wapreadtemplate='$wapreadtemplate',pagemax=$pagemax,isline=$isline,gotoline=$gotoline,isorderby=$isorderby,ordertype=$ordertype";
			$this->db->query('UPDATE ' . $db_table . ' SET ' . $db_set . ' WHERE ' . $db_where);
	...
	function ondeltype() {
		...
		$sql = 'SELECT tid,lng,dirname,dirpath FROM ' . $db_table . ' WHERE ' . $db_where;
		$rs = $this->db->query($sql);
		while ($rsList = $this->db->fetch_assoc($rs)) {

			if (!empty($rsList['dirname'])) {
				$typeRoot = empty($rsList['dirpath']) ? $rsList['dirname'] : $rsList['dirpath'] . '/' . $rsList['dirname'];
				if ($this->CON['is_alonelng']) {
					$typedir = $typepath . $typeRoot;
				} else {
					$typedir = $typepath . $lngdir . '/' . $typeRoot;
				}
