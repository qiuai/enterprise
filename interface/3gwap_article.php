...
class mainpage extends connector {
	...
	function in_list() {
		...
		$template_list = $typeread['waptempalte'];
		$template_index = $typeread['indextemplates'];

		$readtemplate = $typeread['wapreadtemplate'];

		$styleid = $typeread['styleid'];
		$typeread['rsslink'] = $this->get_waplink('typerss', $typeread, admin_LNG);
		$typeread['typelink'] = $this->get_waplink('type', $typeread, admin_LNG);
		...
		if ($styleid <= 2) {
			...
			while ($rsList = $this->db->fetch_assoc($rs)) {
				...
				$rsList['typelink'] = $this->get_waplink('type', $typeread, admin_LNG);
				$rsList['link'] = $this->get_waplink('doc', $rsList, admin_LNG);
				$rsList['buylink'] = $this->get_waplink('buylink', $rsList, admin_LNG);
				$rsList['enqlink'] = $this->get_waplink('enqlink', $rsList, admin_LNG);
			...
			if ($typeread['linkid']) {
				...
				if ($filepage > 1) {
					...
					$readinfo['nlink'] = $nkey <= $filepage ? $this->get_waplink('doc', $readinfo, admin_LNG, $nkey) : null;

					$readinfo['plink'] = $page > 1 ? $this->get_waplink('doc', $readinfo, admin_LNG, $pkey) : null;

					for ($index = 0; $index < $filepage; $index++) {
						$num = $index + 1;
						$pageArray[$index]['num'] = $num;
						$pageArray[$index]['n'] = $num == $page ? 1 : 0;
						$pageArray[$index]['link'] = $this->get_waplink('doc', $readinfo, admin_LNG, $num);
					}
					$outkey = $page - 1;
				...
				$readinfo['buylink'] = $this->get_waplink('buylink', $readinfo, admin_LNG);
				$readinfo['enqlink'] = $this->get_waplink('enqlink', $readinfo, admin_LNG);
				...
				$this->pagetemplate->assign('photo', $albumarray);
			}
	...
	function in_read() {
		...
		if (!empty($readinfo['tags'])) {
			...
			foreach ($tagArray as $key => $value) {
				...
				$newTagArray[$key]['link'] = $this->get_waplink('taglink', array('key' => $value), admin_LNG);
			}
		}
		...
		$typeview['typelink'] = $this->get_waplink('type', $typeview, admin_LNG);
